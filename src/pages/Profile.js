...
import { Image, StyleSheet, View, Dimensions, Pressable, Alert } from 'react-native'
...
const Profile = ({navigation}) => {
    ...
    const ProfileProperty = ({label, info, iconName, iconSize}) => (
        <View style={{
            ...
            <FontAwesome name={iconName} size={iconSize} style={{marginHorizontal:20, color:theme.colors.textColor}} color={theme.colors.textColor} />
            <View style={{
                justifyContent:'center',
                width:'70%'
            }}>
    ...
    const UploadAction = ({iconName, children, onPress}) => (
        ...
        <Text style={styles.uploadModalIconText}>
            {children}
        </Text>
    </View>
    )
  ...
  return (
    ...
    content={
        <>
        <Text onPress={()=> navigation.navigate("ProfileUpdate")} style={{...styles.headerText}} >
            Düzenle
        </Text>
    ...
    <View style={styles.container}>
        <View style={styles.viewUpperPart}
        >
        </View>
        <View style={styles.viewLowerPart}>
            ...
            <View style={styles.profileProperty}>
                <ProfileProperty iconName={"phone"} iconSize={25} label={"Telefon Numarası"} info={"547 846 3574"} />
                <ProfileProperty iconName={"envelope"} iconSize={18} label={"Mail Adresi"} info={"Benbuy@gmail.com.tr"} />
                <ProfileProperty iconName={"lock"} iconSize={25} label={"Şifre"} info={"Görüntülemek için tıklayın"} />
            </View>

            <View style={styles.backButtonView}>
                <Button mode='contained' style={styles.button} textColor='white'
                onPress={() => navigation.navigate("Home")}>Geri</Button>
            </View>
        </View>
        
        <View style={styles.imgContainer}>
            <Pressable onPress={openPhotoModal}>
            <ImageComponent uri={image} style={styles.image} placeHolder={placeHolderImg} />
            </Pressable>
        </View>
    </View>
    <Portal>
        ...
        >
            ...
            content={
                <>
                <Appbar.BackAction onPress={closePhotoModal} size={30} color={theme.colors.textColor} />
                <Appbar.Action icon={'camera'} size={30} color={theme.colors.textColor} onPress={openUploadModal} />
                </>
            }
            />
            <View style={{flex:1, justifyContent:'center', backgroundColor:theme.colors.backgroundColor}}>
            <ImageComponent uri={image} style={styles.modalImage} placeHolder={placeHolderImg} />
            </View>
        </Modal>
        <Modal 
        ...
        >
            <View style={styles.uploadModalContainer}>
                <UploadAction iconName={'folder'} onPress={() => uploadImage('gallery')}>
                    Galeri
                </UploadAction>
